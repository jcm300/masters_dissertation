...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textit{string}: Mantem-se igual tirando os caracteres ``\texttt{<}'', ``\texttt{>}'', 
        ...
        \item \textit{number}: Mantem-se igual;
        \item \textit{boolean}: Mantem-se igual;
...
Assim, apenas é necessário realizar um pedido à \acrshort{api} de dados indicando no cabeçalho \textit{Accept} 
do pedido o formato de saída.

Esta rota da \acrshort{api} possui vários formatos de exportação pelo que se decidiu suportar na \acrshort{api} 
...
inicialmente os dados são obtidos da \acrshort{bd} ou da \textit{cache} da \acrshort{api}. 
