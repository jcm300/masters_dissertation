Para proceder à criação de uma \textit{\acrshort{api} Gateway} é importante primeiro entender o que é e quais os objetivos/aplicações de uma. Além disso, deve-se investigar as várias alternativas existentes no mercado e escolher aquela que melhor se adequa com os nossos requisitos.

É então, na secção do estado da arte descrito o que é uma \textit{\acrshort{api} Gateway} e seus objetivos bem como várias alternativas existentes no mercado.
...
Finalmente, na secção da implementação, descreve-se os passos realizados (desenvolvimento e configuração) para alcançar a \textit{\acrshort{api} Gateway} bem como apresenta-se a arquitetura final desenvolvida.
