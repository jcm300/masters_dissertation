Nesta secção irá ser abordado o estado da arte para a realização dos objetivos referidos na secção de introdução. O objetivo principal desta secção é descrever os conceitos e tecnologias usados, tais como o \acrshort{jwt}, o Autenticação.gov, \textit{Swagger}, entre outros.
...
\begin{itemize}
    ...
    \item Providenciar um recurso \acrshort{http} num \acrshort{uri} conhecido em \texttt{http://example.com/} (são colocados normalmente em \texttt{http://example.com/.well-known/acme-challenge})
\end{itemize}
...
Neste último, o ficheiro de configuração do \textit{Nginx} tem de estar à espera que os ficheiros do certificado estejam em \path{/path/to/keyfile/in/nginx/key.pem} e em \path{/path/to/fullchain/nginx/fullchain.pem}.
