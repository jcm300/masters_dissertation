Nesta dissertação foram também desenvolvidos os mecanismos de \textit{deployment} necessários tanto para a versão sem \textit{Kong} bem como para a versão com \textit{Kong}. Neste capítulo é realizada uma descrição dos passos necessários para realizar o \textit{deployment} de cada versão.
...
De seguida realiza-se a clonagem dos repositórios \texttt{git} que se pretende instalar, muda-se de raíz e de \textit{branch}:
...
Antes de avançar é realizada uma mudança de raiz (\verb|cd deploy|) para a realização dos próximos passos.
...
Antes de recriar as imagens se pretender realizar \textit{push} destas deve alterar as \textit{tags} nos ficheiros de configuração \textit{.env} da(s) imagem(s) que precisará de recriar por forma a não sobescrever \textit{tags}. Fica à escolha do utilizador. Apenas tem de ter em conta que as variáveis ambiente \texttt{GRAPHDB\_IMG}, \texttt{SERVER\_IMG} e \texttt{INTERFACE\_IMG} podem ser utilizadas para gerir as imagens da forma que pretender.
...
Os passos seguintes serão a realização da primeira instalação na nova máquina, bastando para isso seguir a secção~\ref{sec:inst-prim} mas sem realizar o povoamento do \textit{MongoDB}.
...
De seguida realiza-se a clonagem do repositório \texttt{git}, muda-se de raíz e de \textit{branch}:
...
A versão com \textit{Kong} permite um melhor desempenho ou uso do recursos disponíveis desde que corretamente configurada visto possuir \textit{rate limiting} e \textit{cache} dos resultados.

Para comparar a performance foi realizado um teste em que 100 utilizadores fazem dois pedidos, um pedido das classes e quando termina o pedido de uma classe específica. Um utilizador começa os seus pedidos 1 segundo após o anterior. Para realizar este teste foi usado o \textit{Apache JMeter}. Os resultados obtidos foram os seguintes (atenção que a \textit{cache} estava ``quente'' nas duas versões, portanto o pedido das classes deverá ser proveniente da cache):
...
O \textit{throughput} é bastante semelhante entre as duas, contudo o tempo de resposta é menor no caso sem \textit{Kong}, o que faz algum sentido já que a versão com \textit{Kong} (o \textit{Kong} tem embutido o \textit{Nginx}) é mais pesada em termos de componentes em execução.
...
Nesta secção foi também comparada a configuração do \acrshort{https} da interface:
\begin{figure}[H]
    \centering
...
Para testar, cada componente foi executado numa máquina do \textit{Digital Ocean} com \textit{Ubuntu 20.04 x64}, 8 GB de memória \acrshort{ram} e 4 \acrshort{cpu}s. Para traduzir o domínio no \acrshort{ip} da máquina foi usado o \textit{No-IP}\footnote{ver \url{https://www.noip.com/}}.
