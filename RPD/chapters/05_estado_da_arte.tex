...
O conversor de \acrshort{json} para \acrshort{xml} foi contruído sem recorrer a nenhuma biblioteca através do seguinte algoritmo:
...
Da mesma forma que o \acrshort{xml}, o \acrshort{csv} é convertido sem recurso a qualquer biblioteca visto que a conversão a realizar é muito específica a cada objeto \acrshort{json}. Ao contrário do conversor desenvolvido para \acrshort{xml}, o conversor para \acrshort{csv} não converte qualquer objeto para \acrshort{csv} mas apenas um conjunto restrito de objetos \acrshort{json}.
\begin{lstlisting}[language=pseudocode, caption=Algorítmo de conversão de \acrshort{json} para \acrshort{csv}]
\end{lstlisting}
%Algoritmo
%Tentar explicar o que faz
%Exemplo
%CSV

Por fim quanto à exportação da ontologia, das três é a mais simples visto que o \textit{GraphDB}\footnote{} possui funcionalidades de exportação dos triplos presentes numa \acrshort{bd}. O \textit{GraphDB} usa o RDF4J (mais especificamente as suas \acrshort{api}s) por forma a armazenar, consultar bem como suportar várias \textit{query languages} como o \acrshort{sparql} e várias sintaxes \acrshort{rdf}. Desta forma, e seguindo a \acrshort{rest} \acrshort{api} do RDF4J\footnote{Ver \url{https://rdf4j.org/documentation/rest-api/}} é possível obter todos os triplos através da rota \verb|<url do GraphDB>/repositories/<id do repositório>/statements| indicando no cabeçalho \acrshort{http} \textit{Accept} o formato de saída \acrshort{rdf} (\textit{\acrshort{mime} type}\footnote{\textit{Standard} que indica a natureza e o formato de um documento, ficheiro ou conjunto de \textit{bytes}. Ver \href{https://tools.ietf.org/html/rfc6838}{RFC 6838}}) dos triplos. Os formatos \acrshort{rdf} possíveis são \textit{TriG}, \textit{BinaryRDF}, \textit{TriX}, \textit{N-Triples}, \textit{N-Quads}, \acrshort{n3}, \acrshort{rdf}/\acrshort{xml}, \acrshort{rdf}/\acrshort{json}, \acrshort{json-ld} e \acrshort{turtle}. Destes todos apenas tornaremos acessíveis, como já indicado, o \acrshort{turtle} (\texttt{text/turtle}), \acrshort{json-ld} (\texttt{application/ld+json}) e \acrshort{rdf}/\acrshort{xml} (\texttt{application/rdf+xml}).

%falar em como é só atualizado de x em x dias mm que haja vários pedidos
%ontologia, exportações devolvidas e como
