...
O \acrshort{clav} está dividido em duas partes, a interface (\textit{front-end}) presente em \url{http://clav.dglab.gov.pt} e a \acrshort{api} de dados (\textit{back-end} que inclui também duas bases de dados, \textit{GraphDB} e \textit{MongoDB}) presente em \url{http://clav-api.dglab.gov.pt}. Cada parte encontra-se numa máquina diferente.
...
A \acrshort{api} de dados e a interface estavam inicialmente ``juntos'' (aplicação monolítica) onde as rotas eram protegidas contudo, com a separação da aplicação em duas partes, ambas partes deixaram de estar protegidas. Como tal a plataforma \acrshort{clav} já possui duas formas de autenticação, através de chaves \acrshort{api} ou de utilizadores registados.
